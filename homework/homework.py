...
def matriz_c(nombre_set, y_real, y_estimado):
...
def ejecutar_proceso():
    ...
    for nombre, X, y in [("train", X_train, y_train), ("test", X_test, y_test)]:
        ...
        precision, bal_acc, sensibilidad, f1 =metricas(y, predicciones)
        resultados.append({
            "type": "metrics",
            "dataset": nombre,
            "precision": precision,
    ...
    matrices = [
        matriz_c(nombre, y, modelo.predict(X))
        for nombre, X, y in [("train", X_train, y_train), ("test", X_test, y_test)]
    ]
